...
private:
    bool getOdomPose(karto::Pose2& karto_pose, const ros::Time& t, std::string prefix_);
    ...
    void publishMapOdomTF(double transform_publish_period);
    ...
    std::vector<message_filters::Subscriber<sensor_msgs::LaserScan>*> scan_filter_subs;
    std::vector<tf::MessageFilter<sensor_msgs::LaserScan>*> scan_filters;
    // message_filters::Subscriber<sensor_msgs::LaserScan> *scan_filter_sub_1, *scan_filter_sub_2, *scan_filter_sub_3, *scan_filter_sub_4;
    // tf::MessageFilter<sensor_msgs::LaserScan> *scan_filter_1, *scan_filter_2, *scan_filter_3, *scan_filter_4;
    ros::Publisher sst_;
...
//    std::string base_frame_;
    ...
    std::vector<tf::Transform> map_to_odom_records;
...
SlamKarto::SlamKarto() :
        ...
        marker_count_(0)
{   
    // Set tolerance to tf_listener
    tf_.setExtrapolationLimit(ros::Duration(0.02)); 

    // Retrieve parameters
    ros::NodeHandle nh;
    if(!nh.getParam("robot_number", robot_number_)) {  // the global parameter
        robot_number_ = 4;
        ROS_ERROR("\033[92m Not set number of robots!!\033[0m");
    } else {
        ROS_INFO("\033[92m Number of robots: %d \033[0m", robot_number_);
    }
    
    for (int i = 0; i < robot_number_; i++) {
        tf::Transform map_to_odom_x;
        map_to_odom_x.setIdentity();
        map_to_odom_records.push_back(map_to_odom_x);

        message_filters::Subscriber<sensor_msgs::LaserScan> *scan_filter_sub_x;
        scan_filter_subs.push_back(scan_filter_sub_x);

        tf::MessageFilter<sensor_msgs::LaserScan> *scan_filter_x;
        scan_filters.push_back(scan_filter_x);
    }   
    ...
    ROS_DEBUG("transformation tolerance: %f", tmp_tol);
    ...
    for (int i = 0; i < robot_number_; i++) {
        std::string robot_name = "robot_" + std::to_string(i+1);
        scan_filter_subs[i] = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, robot_name + "/filter_scan", 5);
        scan_filters[i] = new tf::MessageFilter<sensor_msgs::LaserScan>(*(scan_filter_subs[i]), tf_, robot_name + "/sensor_laser", 5);
        scan_filters[i]->registerCallback(boost::bind(&SlamKarto::laserCallback, this, _1));
    }
    ...
    // 禁止发布
    // transform_thread_ = new boost::thread(boost::bind(&SlamKarto::publishMapOdomTF, this, transform_publish_period));
...
SlamKarto::~SlamKarto()
{
    ...
    for (int i = 0; i < robot_number_; i++) {
        if (scan_filters[i]) {
            delete scan_filters[i];
        }
        if (scan_filter_subs[i]) {
            delete scan_filter_subs[i];
        }
    }
    // if (scan_filter_1)
    //     delete scan_filter_1;
    // if (scan_filter_sub_1)
    //     delete scan_filter_sub_1;
    // if (scan_filter_2)
    //     delete scan_filter_2;
    // if (scan_filter_sub_2)
    //     delete scan_filter_sub_2;
    // if (scan_filter_3)
    //     delete scan_filter_3;
    // if (scan_filter_sub_3)
    //     delete scan_filter_sub_3;
    // if (scan_filter_4)
    //     delete scan_filter_4;
    // if (scan_filter_sub_4)
    //     delete scan_filter_sub_4;
    if (solver_)
        delete solver_;
    if (mapper_)
        delete mapper_;
...
void
SlamKarto::publishMapOdomTF(double transform_publish_period)
{
...
SlamKarto::publishTransform()
{
    ...
    for (int i = 0; i < robot_number_; i++) {
        tfB_->sendTransform(tf::StampedTransform (map_to_odom_records[i], tf_expiration, map_frame_, "robot_" + std::to_string(i+1) +"/odom"));
    }
//    ignor_first_scan = true;
}
...
SlamKarto::getLaser(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    if(lasers_.find(scan->header.frame_id) == lasers_.end())
    {
        ...
        try
        {
            // tf_.transformPose(scan->header.frame_id.substr(0,8)+"sensor_laser", ident, laser_pose);
            // tf_.transformPose(scan->header.frame_id.substr(0,8)+"sensor_laser", ros::Time(0), ident, ident.frame_id_, laser_pose);
            tf_.transformPose(scan->header.frame_id.substr(0,8)+"sensor_laser", scan->header.stamp, ident, ident.frame_id_, laser_pose);
        }
        ...
        tf::Stamped<tf::Vector3> up(v, scan->header.stamp, scan->header.frame_id.substr(0,8)+"sensor_laser");
...
/**
 * @brief (1) Get robot_n/odom -- robot_n/base_link, should be zero;
 *        (2) Assign initial odom position of robots
 * 
 * @param karto_pose return value of the robot's base_link in the odom frame
 * @param t 
 * @param prefix_ 
 * @return true 
 * @return false 
 */
bool
SlamKarto::getOdomPose(karto::Pose2& karto_pose, const ros::Time& t, std::string prefix_)
{
    // Get the robot's pose
    tf::Stamped<tf::Pose> ident (tf::Transform(tf::createQuaternionFromRPY(0,0,0),
                                               tf::Vector3(0,0,0)), t, prefix_+"sensor_laser");
    tf::Stamped<tf::Transform> odom_pose;
    try
    {
        // tf_.transformPose("odom", ident, odom_pose);
        // tf_.transformPose("odom", ros::Time(0), ident, prefix_+"sensor_laser", odom_pose);
        tf_.transformPose("odom", t, ident, prefix_+"sensor_laser", odom_pose);
    }
    catch(tf::TransformException e)
    {
    ...
    karto_pose =
        karto::Pose2(odom_pose.getOrigin().x(),  // !!!!init_pose!!!
...
{   
    ...
    if(addScan(laser, scan, odom_pose))  // add scan for processing
    {
...
SlamKarto::addScan(karto::LaserRangeFinder* laser,
                   ...
                   karto::Pose2& karto_pose)
{
    if(!getOdomPose(karto_pose, scan->header.stamp, scan->header.frame_id.substr(0,8)))
        return false;
    ...
    {
//        std::cout << "Pose: " << range_scan->GetOdometricPose() << " Corrected Pose: " << range_scan->GetCorrectedPose() << "   " << scan->header.frame_id.substr(0,8) << std::endl;
        ...
        {   
            // Compute the 
            tf_.transformPose("odom",
                              tf::Stamped<tf::Pose> (tf::Transform(tf::createQuaternionFromRPY(0, 0, corrected_pose.GetHeading()), tf::Vector3(corrected_pose.GetX(), corrected_pose.GetY(), 0.0)).inverse(),
                                                     scan->header.stamp, 
                                                     scan->header.frame_id.substr(0,8)+"sensor_laser"),
                              odom_to_map);
            ROS_INFO("\033[92m odom_to_map value: %f, %f, %f   \033[0m", odom_to_map.getOrigin().x(), odom_to_map.getOrigin().y(), odom_to_map.getOrigin().z());
        }
        catch(tf::TransformException e)
        {
