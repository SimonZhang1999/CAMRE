...
#endif
        // tr and increase search angle offset with 20 degrees and do another match
    ...
    forEachAs(std::vector<kt_double>, &yPoses, yIter)
    {
      ...
      forEachAs(std::vector<kt_double>, &xPoses, xIter)
      {
        ...
        for (kt_int32u angleIndex = 0; angleIndex < nAngles; angleIndex++)
        {
          ...
          {
            // simple model (approximate Gaussian) to take odomet into account
...
#endif
#ifdef gazebo
        Pose2 previousPose = pSensorManager->GetScans(rCandidateSensorName)[0]->GetCorrectedPose();
        Pose2 currentPose = pScan->GetCorrectedPose();
        bestPose.SetX(currentPose.GetX());
        bestPose.SetY(currentPose.GetY());
        covariance(0, 0) = INIT_ESTIMATE;  // XX
        covariance(1, 1) = INIT_ESTIMATE;  // YY
        covariance(2, 2) = 4 * math::Square(m_pMapper->m_pCoarseAngleResolution->GetValue());  // TH*TH
#endif
  ...
  kt_bool MapperGraph::TryCloseLoop(LocalizedRangeScan* pScan, const Name& rSensorName)
  {
    ...
    while (!candidateChain.empty())
    {
          ...
          (covariance(1, 1) < m_pMapper->m_pLoopMatchMaximumVarianceCoarse->GetValue()) )
      {
          ...
          std::cout << "var: " << covariance(0, 0) << ",  " << covariance(1, 1) << " (< " << m_pMapper->m_pLoopMatchMaximumVarianceCoarse->GetValue() << ")";LocalizedRangeScan tmpScan(pScan->GetSensorName(), pScan->GetRangeReadingsVector());
...
//           }LBP_node;
//           karto::Graph<karto::LocalizedRangeScan>::VertexMap LBP_vertex = m_pMapper->m_pGraph->GetVertices();
//           int begin = 0;
//           std::vector<int> queue;
//           queue.push_back(pScan->GetStateId());
//           while(queue[begin] != 0 ) {
// //              std::cout << "Pose = " << LBP_vertex[pScan->GetSensorName()][queue[begin]]->GetObject()->GetCorrectedPose();
//               std::vector<Vertex<LocalizedRangeScan>*> LBP_advertex = LBP_vertex[pScan->GetSensorName()][queue[begin]]->GetAdjacentVertices();
//               const_forEach(std::vector<Edge<LocalizedRangeScan>*>, &(LBP_vertex[pScan->GetSensorName()][queue[begin]]->GetEdges()))
//               {
//                   Edge<LocalizedRangeScan>* pEdge = *iter;
//                   queue.push_back(pEdge->GetSource()->GetObject()->GetStateId());
//                   karto::LinkInfo* LBP_label = dynamic_cast<karto::LinkInfo*> (pEdge->GetLabel());
//                   std::cout << "From " << queue[begin] << " To " << queue[queue.size()-1] << "Mean = " << LBP_label->GetPoseDifference()
//                             << " Convariance " << LBP_label->GetCovariance();
//               }
      ...
      if (m_pUseScanMatching->GetValue() && pLastScan != NULL)
      {
        ...
        m_pSequentialScanMatcher->MatchScan(pScan,
                                            m_pMapperSensorManager->GetRunningScans(pScan->GetSensorName()),
                                                                                    bestPose,
                                                                                    covariance);                                                                        
        // pScan->SetSensorPose(bestPose);  // 这里也会调用SetCorrectedPose
