...
# Map data:  100 occupied      -1 unknown       0 free
def gridNeighborHasObstacle(mapData, Xp, step):
    """ Check whether neighboring grids has obstacles. """
    resolution = mapData.info.resolution
    Xstartx = mapData.info.origin.position.x
    Xstarty = mapData.info.origin.position.y
    width = mapData.info.width
    ...
    for i in range(1, step+1):
        low_index = index - width*i
        high_index = index + width*i
        neighbor_indices.append(low_index)
        neighbor_indices.append(high_index)
        for j in range(1, step+1):
            neighbor_indices.append(low_index - j)
            neighbor_indices.append(low_index + j)
            neighbor_indices.append(high_index - j)
            neighbor_indices.append(high_index + 1)
    for j in range(1, step+1):
        neighbor_indices.append(index + j)
        neighbor_indices.append(index - j)

    for idx in neighbor_indices:
        if int(idx) < 0 or int(idx) >= len(Data):
            continue
        if Data[int(idx)] > 70:
            return True
    return False
    ...
    with open(filename) as fp:
        ...
        for x in lines:
            ...
            if edge_type == "VERTEX_SE2":
                ...
                # print(x.split(' ')[6:])
                FIM = np.float_(x.split(' ')[6:-1])
                # EDGE_SE2 1 3 0.0124617 0.00610883 0.0041659 3535.6 -862.417 0 2194.91 0 16980 
                # FIM_values = x.split(' ')[7:13]  # Get the values to convert to float
                # FIM = np.zeros(6)  # Initialize an array to store the converted values
                # if all(FIM_values):  # Check if all values are non-empty
                #     FIM = np.float_(FIM_values)  # Convert non-empty values to float
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# compute the map information at one point
def cellInformation(mapData, point, r):
    ...
    #TODO: Here use the number of occupied cells to predict the potential for loop closure?
