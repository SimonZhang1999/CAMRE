...
MSPlanner::MSPlanner(const Config &conf, ros::NodeHandle &nh, std::shared_ptr<SDFmap> map):config_(conf){
    nh_ = nh;
    map_ = map;
    scc_ = std::make_shared<star_convex_handle::StarConvexHandler>(nh_);
    std::string robot_ns = ros::names::parentNamespace(nh_.getNamespace()); 
    mincoinitPath = nh_.advertise<nav_msgs::Path>(robot_ns + "/visualizer/mincoinitPath",10);
    ...
    readParam(ros::this_node::getName()+ "/safeIrDis", safeIrDis_);
    safeDis = safeDis_;
    safeIrDis = safeIrDis_;
...
bool MSPlanner::optimizer(){
    ...
    // if (cost > 1e4) {
    //     ROS_ERROR("Cost is too large, may be infeasible!!!");
    //     return false;
    // }
    return true;
}
...
void MSPlanner::attachPenaltyFunctional(double &cost){
    ...
    double violaAcc, violaAlp, violaPos, violaIr, violaSCP,violaMom, violaCenAcc;
    double violaAccPena, violaAlpPena, violaPosPena, violaIrPena, violaSCPPena, violaSCPPenaDAppro, violaMomPena, violaCenAccPena;
    double violaAccPenaD, violaAlpPenaD, violaPosPenaD, violaIrPenaD, violaSCPPenaD, violaMomPenaD, violaCenAccPenaD;
    double gradViolaAT, gradViolaDOT, gradViolaPt, gradViolaIr, gradViolaMt, gradViolaCAt;
    ...
    Eigen::Vector2d gradIr, gradCostIr;
    Eigen::Vector2d gradSCP, gradCostSCP;
    ...
    // ===================== NEW: SCP only on first 10% of whole trajectory (by time) =====================
    const double totalT  = pieceTime.sum();
    const double scpTmax = 0.85 * totalT;
    double sumT = 0.0; // cumulative time at the start of current piece
    // ====================================================================================================
    ...
    for(int i=0; i<TrajNum; i++){
        ...
        for(int j=0; j<=SamNumEachPart; j++){
            if(j%2 == 0){
                ...
                if(config_.if_directly_constrain_v_omega_){
                    violaVel = dsigma.y() * dsigma.y() - config_.max_vel_ * scale_down_param_ * config_.max_vel_ * scale_down_param_;
                    if(violaVel > 0){
                    ...
                    for(int omg_sym = -1; omg_sym <= 1; omg_sym += 2){
                        violaMom = omg_sym * config_.max_vel_ * scale_down_param_ * dsigma.x() + config_.max_omega_ * dsigma.y() - config_.max_vel_ * scale_down_param_ * config_.max_omega_;
                        if(violaMom > 0){
                            positiveSmoothedL1(violaMom, violaMomPena, violaMomPenaD);
                            gradViolaMt = Alpha * (omg_sym * config_.max_vel_ * scale_down_param_ * ddsigma.x() + config_.max_omega_ * ddsigma.y());
                            gradBeta(1,0) += omgstep * penaltyWt.moment_weight * violaMomPenaD * omg_sym * config_.max_vel_ * scale_down_param_;
                ...
                // ===================== NEW: apply SCP only on global first 10% time =====================
                const double global_t = sumT + (static_cast<double>(j) * halfstep);
                if (global_t <= scpTmax) {
                    // SCP constraints
                    for (int r_index = 0; r_index < connected_robots_poses_.size(); r_index++) {
                        if (costSCPset_[r_index] > 0.0) {
                            positiveSmoothedL1(costSCPset_[r_index], violaSCPPena, violaSCPPenaD);
                            cost += omgstep * penaltyWt.collision_weight * std::pow(violaSCPPena, 3);
                            // 按你的要求：不修改这里梯度与 cost 不一致的写法
                            gradCostSCP = omgstep * penaltyWt.collision_weight * violaSCPPenaD * gradSCPset_[r_index];

                            VecCoeffChainX.head(i*(SamNumEachPart+1) + j + 1).array() += gradCostSCP.x();
                            VecCoeffChainY.head(i*(SamNumEachPart+1) + j + 1).array() += gradCostSCP.y();
                        }
                    }
                }
                // =================================================================================================

                // Inter-Robot Collision Avoidance Contraints
                for (int r_index = 0; r_index < other_robots_poses_.size(); r_index++) {

                    Eigen::Vector2d diff_ir = CurrentPointXY - other_robots_poses_[r_index];
                    double dist_ir = diff_ir.norm();

                    violaIr = safeIrDis - dist_ir;

                    if (violaIr > 0.0) {
                        positiveSmoothedL1(violaIr, violaIrPena, violaIrPenaD);

                        cost += omgstep * penaltyWt.collision_weight * violaIrPena;

                        if (dist_ir > 1e-6) {
                            gradIr = -diff_ir / dist_ir;
                        } else {
                            gradIr.setZero();
                        }

                        gradCostIr = omgstep * penaltyWt.collision_weight * violaIrPenaD * gradIr;

                        VecCoeffChainX.head(i*(SamNumEachPart+1) + j + 1).array() += gradCostIr.x();
                        VecCoeffChainY.head(i*(SamNumEachPart+1) + j + 1).array() += gradCostIr.y();
                    }
                }
        ...
        // ===================== NEW: update piece start time for next segment =====================
        sumT += pieceTime[i];
        // ========================================================================================
    }
            ...
            if(j%2 == 0){
                ...
                for(int omg_sym = -1; omg_sym <= 1; omg_sym += 2){
                    ...
                    if(violaMom > 0){
                        positiveSmoothedL1(violaMom, violaMomPena, violaMomPenaD);
                        gradViolaMt = Alpha * (omg_sym * config_.max_vel_ * scale_down_param_ * ddsigma.x() + config_.max_omega_ * ddsigma.y());
                        gradBeta(1,0) += omg * step * PathpenaltyWt.moment_weight * violaMomPenaD * omg_sym * config_.max_vel_ * scale_down_param_;
