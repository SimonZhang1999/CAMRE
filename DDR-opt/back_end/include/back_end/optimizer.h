...
private:
    ...
    std::shared_ptr<star_convex_handle::StarConvexHandler> scc_;
    ...
    double scale_down_param_ = 1.0;
    std::vector<double> costSCPset_;
    std::vector<Eigen::Vector2d> gradSCPset_;
...
public:
    ...
    // Obtain the scale down parameter
    inline void set_scale_down_param(const double& scale_down_param) {
        scale_down_param_ = scale_down_param;
    }
    inline double get_scale_down_param() {
        return scale_down_param_;
    }
    // Obtain the current states of the other robots
    inline void set_other_robots_states(const std::vector<Eigen::Vector2d>& or_poses) {
        other_robots_poses_ = std::move(or_poses);
    }
    // Obtain the current states of the connected robots
    inline void set_connected_robots_states(const std::vector<Eigen::Vector2d>& cr_poses) {
        connected_robots_poses_ = std::move(cr_poses);
    }
    // Obtain the current convex hull of the connected robots
    inline void set_connected_robots_convexhulls(const std::vector<std::vector<Eigen::Vector2d>>& cr_convexhulls) {
        connected_robots_convexhull_ = std::move(cr_convexhulls);
    }
    // Obtain the current convex hull of the connected robots
    inline void set_connected_robots_scps(const std::vector<Eigen::Vector3d>& scp_info) {
        gradSCPset_.resize(scp_info.size());
        costSCPset_.resize(scp_info.size());
        for (int i = 0; i < scp_info.size(); i++) {
            costSCPset_[i] = scp_info[i](0);
            gradSCPset_[i] = {scp_info[i](1), scp_info[i](2)};
        }
    }
    // Optimization
    bool optimizer();
