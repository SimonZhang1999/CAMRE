...
MpcController::MpcController(const ros::NodeHandle &nh){
    ...
    nh.param(robot_ns + "/mpc/du_threshold", du_th, -1.0);
    nh.param(robot_ns + "/mpc/dt", dt, -1.0);
    nh.param(robot_ns + "/mpc/max_iter", max_iter, -1);
    nh.param(robot_ns + "/mpc/predict_steps", T, -1);
    nh.param(robot_ns + "/mpc/max_omega", max_omega, -1.0);
    nh.param(robot_ns + "/mpc/max_domega", max_domega, -1.0);
    nh.param(robot_ns + "/mpc/max_vel", max_speed, -1.0);
    nh.param(robot_ns + "/mpc/min_vel", min_speed, -1.0);
    nh.param(robot_ns + "/mpc/max_acc", max_accel, -1.0);
    nh.param(robot_ns + "/mpc/delay_num", delay_num, -1);
    nh.param(robot_ns + "/mpc/cmd_timer_rate", cmd_timer_rate_, 100.0);
    nh.param(robot_ns + "/mpc/max_mpc_time", max_mpc_time_, 10.0);

    nh.param(robot_ns + "/mpc/if_mpc", use_mpc_, true);

    emergency_stop_sub_ = nh_.subscribe<std_msgs::Bool>(robot_ns + "/planner/emergency_stop", 1, &MpcController::emergencyStop, this);
    scale_down_sub_ = nh_.subscribe<std_msgs::Float64>(robot_ns + "/scale_down_pram", 1, &MpcController::sdvCb, this);


    nh.param<std::vector<double>>(robot_ns + "/mpc/matrix_q", Q, std::vector<double>());
    nh.param<std::vector<double>>(robot_ns + "/mpc/matrix_r", R, std::vector<double>());
    nh.param<std::vector<double>>(robot_ns + "/mpc/matrix_rd", Rd, std::vector<double>());
    ...
    nh.param(robot_ns + "/mpc/state_seq_res", state_seq_res, 1.0);
    nh.param(robot_ns + "/mpc/Integral_appr_resInt", Integral_appr_resInt, 10);
    traj_.setRes(state_seq_res, Integral_appr_resInt);
    new_traj_.setRes(state_seq_res, Integral_appr_resInt);

    traj_sub_ = nh_.subscribe<carstatemsgs::Polynome>(robot_ns + "/traj", 1, &MpcController::TrajCallback, this);
    // odom_sub_ = nh_.subscribe<carstatemsgs::CarState>("odom", 1, &MpcController::OdomCallback, this);
    vel_sub_ = nh_.subscribe<nav_msgs::Odometry>(robot_ns + "/RosAria/odom", 10, &MpcController::OdomCallbackPose, this);
    sequence_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/mpc/sequence",1);
    cmd_pub_ = nh_.advertise<carstatemsgs::CarState>("cmd",1);
    cmd_vel_pub_ = nh_.advertise<geometry_msgs::Twist>(robot_ns + "/RosAria/cmd_vel", 1000);
...
void MpcController::OdomCallbackPose(const nav_msgs::Odometry::ConstPtr &msg){
    has_odom = true;
    ...
    try
    {
        ...
        transformStamped = tf_buffer.lookupTransform(global_frame_, msg->child_frame_id, msg->header.stamp, ros::Duration(0.1));
        ...
        now_state.y = y;
        now_state.theta = yaw;
        now_state.v = msg->twist.twist.linear.x;
        now_state.w = msg->twist.twist.angular.z;
        // now_state.w = 0.0;
...
// void MpcController::PoseCb(const sensor_msgs::PointCloud2::ConstPtr& msg){
//     static tf2_ros::Buffer tf_buffer;
//     static tf2_ros::TransformListener tf_listener(tf_buffer);
//     try
//     {
//         has_odom = true;
//         geometry_msgs::TransformStamped transformStamped;
//         transformStamped = tf_buffer.lookupTransform(global_frame_, msg->header.frame_id, msg->header.stamp, ros::Duration(0.1));
//         const double x = transformStamped.transform.translation.x;
//         const double y = transformStamped.transform.translation.y;
//         const double yaw = tf::getYaw(transformStamped.transform.rotation);
//         now_state.x = x;
...
// void MpcController::VelocityCallbackPose(const nav_msgs::Odometry::ConstPtr &msg){
//     now_state.v = msg->twist.twist.linear.x, 0.0, 0.0;
//     now_state.w = msg->twist.twist.angular.z, 0.0, 0.0;
// }
...
void MpcController::CmdCallback(const ros::TimerEvent& event){
    if (!has_odom || !receive_traj_ || !has_target_type_)
        return;
    ...
    if (at_goal){
        ...
        if(use_mpc_){
            ...
            geometry_msgs::Twist cmd_vel;
            cmd_vel.linear.x = curr_v.y();
            cmd_vel.angular.z = curr_v.x();
            cmd_vel_pub_.publish(cmd_vel);
        }
...
void MpcController::stateTrans(MPCState& s, double a, double yaw_dot)
{
    ...
    if (s.v >= (max_speed * scale_down_param_))
    {
        s.v = (max_speed * scale_down_param_);
    }
...
void MpcController::solveMPCV(void)
{
    ...
    {
        lowerBound[i] = (-max_speed * scale_down_param_);
        lowerBound[i+1] = -max_omega;
        upperBound[i] = (max_speed * scale_down_param_);
...
void MpcController::sdvCb(const std_msgs::Float64::ConstPtr& msg) {
  has_target_type_ = true;
  scale_down_param_ = msg->data;
}
