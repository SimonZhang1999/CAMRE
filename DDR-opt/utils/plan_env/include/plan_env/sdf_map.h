...
class SDFmap
{
  private:
    ...
    ros::Publisher pub_NeiPC_;
    ros::Publisher pub_fused_cloud_;
    ...
    nav_msgs::OccupancyGrid latest_occ_grid_;
    bool has_occ_grid_ = false;
    std::mutex occ_grid_mutex_;

    bool fuse_occ_grid_to_cloud_ = true;
    int occ_grid_threshold_ = 50;
    int occ_grid_stride_ = 1;
  ...
  public:
    ...
    double local_map_size_x_ = DBL_MAX;   
    double local_map_size_y_ = DBL_MAX;  
    ...
    double r_min_, r_max_, safe_dis_;
    ...
    SDFmap(const ros::NodeHandle &nh){
      nh_ = nh;
      std::string robot_ns_ = ros::names::parentNamespace(nh_.getNamespace()); 

      // debug
      debug_maker = nh_.advertise<visualization_msgs::MarkerArray>(robot_ns_+"/SDFmap/debug",1);

      nh_.param<double>(ros::this_node::getName()+"/gridmap_interval",grid_interval_,0.1);
      inv_grid_interval_ = 1/grid_interval_;
  
      pub_gridmap_ = nh_.advertise<sensor_msgs::PointCloud2>(robot_ns_+"/SDFmap/gridmap",1);
      pub_ESDF_ = nh_.advertise<sensor_msgs::PointCloud2>(robot_ns_+"/SDFmap/ESDF",1);
      
      pub_gradESDF_ = nh_.advertise<visualization_msgs::MarkerArray>(robot_ns_+"/SDFmap/ESDFGrad",1);

      pub_NeiPC_ = nh_.advertise<sensor_msgs::PointCloud2>(robot_ns_+"/NeighborPointcloud",1);
      pub_fused_cloud_ = nh_.advertise<sensor_msgs::PointCloud2>("fused_cloud", 1);
      ...
      occ_grid_sub_ = nh_.subscribe("/map", 1, &SDFmap::occGridCallback, this);
      nh_.param<double>(ros::this_node::getName()+"/detection_range",detection_range_,5.0);
      
      nh_.param<double>(ros::this_node::getName()+"/local_map_size_x",local_map_size_x_, 20);
      nh_.param<double>(ros::this_node::getName()+"/local_map_size_y",local_map_size_y_, 20);
      ...
      nh_.param<double>(ros::this_node::getName()+"/pc_r_min",r_min_,2.0);
      nh_.param<double>(ros::this_node::getName()+"/pc_r_max",r_max_,2.1);
      nh_.param<double>(ros::this_node::getName()+"/pc_safe_dis",safe_dis_,2.0);
      ...
      GLX_SIZE_  = static_cast<int>(std::ceil(local_map_size_x_ / grid_interval_));
      GLY_SIZE_  = static_cast<int>(std::ceil(local_map_size_y_ / grid_interval_));
      GLXY_SIZE_ = GLX_SIZE_ * GLY_SIZE_;
    ...
    void appendLocalOccupiedCellsFromGrid(pcl::PointCloud<pcl::PointXYZ>& cloud);
