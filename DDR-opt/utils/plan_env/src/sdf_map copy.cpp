...
void SDFmap::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &msg){
  ...
  try {
      transformStamped = tf_buffer.lookupTransform("map", msg->header.frame_id,
...
void SDFmap::updateOccupancyCallback(const ros::TimerEvent& /*event*/){
  if(!local_map_inited_) return;  // 防御

  // 让“global_*”变成当前局部窗口在 map 坐标系下的边界（以机器人为中心）
  ...
  // inv_grid_interval_ 你应该早就有了；如果 grid_interval_ 会动态改，也可在这更新
  inv_grid_interval_ = 1.0 / grid_interval_;

  if(!occ_need_update_) return;
  // 每次都从“未知/最小log-odds”开始重建局部窗口
  // std::fill(gridmap_.begin(), gridmap_.end(), Unknown);
  ...
  // distance_buffer_all_ 不用在这里填，ESDF会覆盖更新区域
  ...
  if(!if_perspective_){
    ...
    // 这些现在是“局部更新范围大小”，但底层 buffer 是 GLX/GLY 的窗口固定大小
    X_SIZE_ = GLX_SIZE_;
    Y_SIZE_ = GLY_SIZE_;
    XY_SIZE_ = GLXY_SIZE_;
...
void SDFmap::updateESDFCallback(const ros::TimerEvent& /*event*/){
  ...
  // ROS_WARN_THROTTLE(1.0, "odom_pos: %.3f %.3f %.3f, esdf_need_update=%d occ_need_update=%d cloud=%zu",
  //                 odom_pos_.x(), odom_pos_.y(), odom_pos_.z(),
  //                 (int)esdf_need_update_, (int)occ_need_update_, cloud_.points.size());
...
void SDFmap::publish_Neighboring_PC(){
  pcl::PointCloud<pcl::PointXYZ> cloud_vis;   // 只含 x y z
  sensor_msgs::PointCloud2 surf_vis;

  const double r_min = 2;   
  const double r_max = 2.1;   
  const double safe_esdf = 2.0;   
  const double r1 = r_min * r_min;
  const double r2 = r_max * r_max;
